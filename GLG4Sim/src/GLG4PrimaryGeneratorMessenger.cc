  ...
  if( command == ListCmd )
   {
     ...
     for (int i=0; i<myAction->theNumEventTypes; i++) {
       G4cout << G4std::setw(4)
	      ...
	      << G4std::setw(4)
	      << myAction->GetVertexCodeForEventType(i) << "  "
  ...
  else if( command == RateCmd )
   {
     ...
     if (strlen(id)==0) {
       ...
       G4cout << "Rates:\n";
       for (int i=0; i<myAction->theNumEventTypes; i++) {
	 G4cout << G4std::setw(3) << i << " "
		<< G4std::setw(7) << myAction->GetEventRate(i)/(1./second)
		<< (myAction->GetEventTriggerCondition(i) ? " * " : "   ")
		<< myAction->GetEventTypeName(i)
		<< G4endl;
       }
       G4cout << " * indicates \"pileup-only\" generator condition set." << G4endl;
     }
     else {
	 ...
	 G4cout << "Index must be non-negative and less than "
		...
		<< "  (No match for \"" << id << "\" was found.)"
		<< G4endl;
       }
       else {
	 is >> rate;
	 if (is.fail()) {
	   G4cout << G4std::setw(3) << index << " "
		  << G4std::setw(7) << myAction->GetEventRate(index)/(1./second)
		  << "  " << myAction->GetEventTypeName(index)
		  << G4endl;
	 }
	 else if (rate < 0.0) {
	 ...
	 is >> flag;
	 if (!is.fail())
	   myAction->SetEventTriggerCondition(index, flag);
       }
     }
   }
	...
	do { // this "do" is just to allow "break" to exit this block
	  ...
	  is >> newParticleName;
	  if (is.fail() || newParticleName.length()==0)
	    break;
	  ...
	  if (is.fail())
	    goto do_vtx;
	  os_pos << "/generator/pos/set " << myAction->kGunPosIndex << " \""
		 << x << ' ' << y << ' ' << z << "\"" << G4std::ends;
	...
	do_vtx:
      ...
      G4cout << "Current TestGun settings:\n\t"
	     << GetCurrentValue(command) << G4endl;
    }
	...
	return;
      ...
      myAction->GetVertexGenerator(index)
	->SetState( newValues.substr(is.tellg()) );
    }
	...
	return;
      ...
      myAction->GetPositionGenerator(index)
	->SetState( newValues.substr(is.tellg()) );
    }
  else if( command == EventWindowCmd )
   {
     if (newValues.length()==0)
       {
	 G4cout << "Current window is "
		<< myAction->GetEventWindow()/ns << " ns" << G4endl;
       }
     else
       {
	 istringstream is((const char *)newValues);
  ...
  else if( command == ChainClipCmd )
   {
     if (newValues.length()==0)
       {
	 G4cout << "Current clip time is "
		<< myAction->GetChainClip()/ns << " ns" << G4endl;
       }
     else
       {
	 istringstream is((const char *)newValues);
...
{ 
  ...
  if( command == GunCmd )
   {
     G4String vstring= myAction->GetVertexGenerator(myAction->kGunVtxIndex)
       ->GetState();
     G4String pstring= myAction->GetPositionGenerator(myAction->kGunPosIndex)
       ->GetState();
  ...
  if (command == VtxSetCmd)
	...
	os << index << '\t'
	   << myAction->GetVertexGenerator(index)->GetState() << '\n';
      os << G4std::ends;
      G4String rv(os.str());
      //os.freeze(0);
      return rv;      
	...
	os << index << '\t'
	   << myAction->GetPositionGenerator(index)->GetState() << '\n';
      os << G4std::ends;
      G4String rv(os.str());
      //os.freeze(0);
      return rv;      
