/** @file
   ...
   $Id: GLG4VertexGen.cc,v 1.2 2013/11/09 23:48:54 jslee Exp $
...
void GLG4VertexGen_Gun::SetState(G4String newValues)
{
  ...
  db[(_dbname + ".pdgcode").c_str()] = _pDef->GetPDGEncoding();
  ...
  is >> x;
  if (is.fail()) {
    db[(_dbname + ".px").c_str()] = _mom.x();
    db[(_dbname + ".py").c_str()] = _mom.y();
    db[(_dbname + ".pz").c_str()] = _mom.z();
    db[(_dbname + ".KE").c_str()] = _ke;
    return;
  }
  if (x <= 0.0) {
    p_renorm = 0.0;
  ...
  db[(_dbname + ".polx").c_str()] = _pol.x();
  db[(_dbname + ".poly").c_str()] = _pol.y();
  db[(_dbname + ".polz").c_str()] = _pol.z();
...
G4String GLG4VertexGen_Gun::GetState()
{
  ...
  os << _pDef->GetParticleName() << '\t' << _mom.x() << ' ' << _mom.y() << ' '
     << _mom.z() << '\t' << _ke << '\t' << _pol.x() << ' ' << _pol.y() << ' '
     << _pol.z() << '\t' << _multiplicity << G4std::ends;
  G4String rv(os.str());
...
void GLG4VertexGen_HEPEvt::Close()
{
  if (_file) {
    if (_isPipe) {
      ...
      if (status != 0) {
        G4cerr << "HEPEvt input pipe from " << _filename << " gave status "
               << status << " on close." << G4endl;
      }
      _file = 0;
      _isPipe = false;
      _filename = "";
...
void GLG4VertexGen_HEPEvt::GetDataLine(char * buffer, size_t size)
{
  ...
  for (;;) {
    ...
    if (fgets(buffer, size, _file) != buffer) {
      ...
      G4cerr << (feof(_file) ? "End-of_file reached" : "Failure") << " on "
             << _filename << " at " << ftell(_file);
      if (rewind_count == 0) {
        if (_isPipe == false) {
          ...
          pclose(_file);
          _file =
              popen(_filename.substr(0, _filename.length() - 1).c_str(), "r");
        }
        rewind_count++;
        continue;
      }
    ...
    if (strcmp(firstword, "STATE") == 0) {
      ...
      while (fgets(buffer, size, _file) == buffer) {
        ...
        if (nscan == 2) db[((_dbname + ".") + firstword).c_str()] = value;
        else {
          G4cout << "Warning, bad STATE line in " << _filename << ": " << buffer
                 << G4endl;
        }
      }
      continue;
...
void GLG4VertexGen_HEPEvt::GeneratePrimaryVertex(G4Event * argEvent)
{
  ...
  if (_file == 0) {
    G4cerr << "Unexpected end of file in " << _filename << ", expecting NHEP."
           << G4endl;
    Close();
    return;
  }
  ...
  for (int IHEP = 0; IHEP < NHEP; IHEP++) {
    ...
    G4double req_vertex_dT =
        req_novalue;     // vertex _delta_ time (in ns, a GLG4HEPEvt convention)
    ...
    is >> ISTHEP >> IDHEP >> JDAHEP1 >> JDAHEP2 >> PHEP1 >> PHEP2 >> PHEP3 >>
        PHEP5 >> req_vertex_dT >> req_vertexX >> req_vertexY >>
        req_vertexZ // note order!
        >> polx >> poly >> polz;
    ...
    if (IDHEP > kIonCodeOffset && IDHEP < kIonCodeOffset + 99999) {
      ...
      if (g4code == 0) {
        G4cerr << "Warning: GLG4HEPEvt could not find Ion Z=" << Z << " A=" << A
               << " code=" << IDHEP << G4endl;
        particle =
            new G4PrimaryParticle(IDHEP, PHEP1 * energy_unit,
                                  PHEP2 * energy_unit, PHEP3 * energy_unit);
      }
      else {
        particle =
            new G4PrimaryParticle(g4code, PHEP1 * energy_unit,
                                  PHEP2 * energy_unit, PHEP3 * energy_unit);
      }
    }
      ...
      particle = new G4PrimaryParticle(G4OpticalPhoton::OpticalPhoton(),
                                       PHEP1 * energy_unit, PHEP2 * energy_unit,
                                       PHEP3 * energy_unit);
    }
    else {
      particle = new G4PrimaryParticle(
          IDHEP, PHEP1 * energy_unit, PHEP2 * energy_unit, PHEP3 * energy_unit);
    }
...
void GLG4VertexGen_Stack::GeneratePrimaryVertex(G4Event * argEvent)
{
  ...
  G4double eventEndTime =
      fGLG4PGA->GetUniversalTime() + fGLG4PGA->GetEventWindow();

  while (!_stack.empty()) {
    _stack_iterator_t b = _stack.begin();
    G4PrimaryVertex * v = *b;
    if (v->GetT0() > eventEndTime) break;
  ...
  if (!_stack.empty()) {
    G4double nextTime = (*(_stack.begin()))->GetT0();
    fGLG4PGA->NotifyTimeToNextStackedEvent(nextTime -
                                           fGLG4PGA->GetUniversalTime());
  }
}
...
void GLG4VertexGen_Stack::StackIt(const G4Track * track)
{
  G4PrimaryVertex * vertex = new G4PrimaryVertex(
      track->GetPosition(),
      track->GetGlobalTime() + fGLG4PGA->GetUniversalTime());
  ...
  _stack.insert(vertex);
}

void GLG4VertexGen_Stack::SetState(G4String)
{
  G4cout << "GLG4VertexGen_Stack has no user-settable state." << G4endl;
  G4cout << "Current status is as follows:" << G4endl;
  int i = 0;
  for (_stack_iterator_t it = _stack.begin(); it != _stack.end(); it++, i++) {
    G4PrimaryVertex * v = *it;
    G4cout << i << "\t" << v << G4endl;
  }
  G4cout << "Number of particles on stack: " << i << G4endl;
}

G4String GLG4VertexGen_Stack::GetState()
{
  G4String rv("");
  return rv;
}
...
SetState(G4String newValues)
{
  ...
  G4std::istrstream is(newValues.c_str());
}
