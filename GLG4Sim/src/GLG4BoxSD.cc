...
// $Id: GLG4BoxSD.cc,v 1.2 2013/11/11 01:22:25 jslee Exp $
//
//  GLG4BoxSD.cc
//
...
G4bool GLG4BoxSD::ProcessHits(G4Step * aStep, G4TouchableHistory*)
{
  ...
  if(edep > 0.0)
    for (G4int i = iz1; i <= iz2; i++)      // note "<=" implies numseg segments
      h_edep[i] += edep/numseg;

  G4ParticleDefinition* pdef= pTrack->GetDefinition();
  if(pdef->GetPDGCharge() == 0){
    if(e0 > gCut) 
      for (G4int i = iz1; i < iz2; i++)     // note "<" for only boundary-crossings
	h_ng[i]++;
  }
  else {
    if(e0 > eCut) 
...
void GLG4BoxSD::PrintAll()
{
  ...
  dump_histo(h_ng, nbin_boxsd, 0);
  G4cout<<"# charged vs. 2*rad.length"<<G4endl;
  dump_histo(h_ne, nbin_boxsd, 0);
  G4cout<<"# edep vs. 2*rad.length"<<G4endl;
  dump_histo(h_edep, nbin_boxsd, 0);
