				       ...
				       )
{
  ...
  for (; v!=NULL; v=v->GetNext()) {
    v->SetPosition( v->GetX0() + offset.x(),
		    v->GetY0() + offset.y(),
		    v->GetZ0() + offset.z() );
    v->SetT0( v->GetT0() + dt );
...
void
GLG4VPosGen::Strip( G4String &s, const char *stripchars )
{
...
GLG4PosGen_PointPaintFill::GLG4PosGen_PointPaintFill(const char *arg_dbname)
  :  GLG4VPosGen(arg_dbname),
     _fixedPos(0.,0.,0.), _mode(kPoint), _thickness(0.),
     _pVolumeName("!"), _pVolume(0),
     _materialName(""), _material(0),
...
void GLG4PosGen_PointPaintFill::GeneratePosition( G4ThreeVector *argResult )
{
  ...
  if (pv == 0) {
    G4cerr << "GLG4PosGen_PointPaintFill: "
      "Could not find any volume at " << _fixedPos << G4endl;
    return;
  }
