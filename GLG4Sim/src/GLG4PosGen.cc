/**@file
   ...
   $Id: GLG4PosGen.cc,v 1.2 2013/11/09 23:48:54 jslee Exp $
                                          ...
                                          double event_rate, double dt)
{
  ...
  for (; v != NULL; v = v->GetNext()) {
    v->SetPosition(v->GetX0() + offset.x(), v->GetY0() + offset.y(),
                   v->GetZ0() + offset.z());
    v->SetT0(v->GetT0() + dt);
...
void GLG4VPosGen::Strip(G4String & s, const char * stripchars)
{
...
GLG4PosGen_PointPaintFill::GLG4PosGen_PointPaintFill(const char * arg_dbname)
  : GLG4VPosGen(arg_dbname),
    _fixedPos(0., 0., 0.),
    _mode(kPoint),
    _thickness(0.),
    _pVolumeName("!"),
...
void GLG4PosGen_PointPaintFill::GeneratePosition(G4ThreeVector * argResult)
{
  ...
  if (pv == 0) {
    G4cerr << "GLG4PosGen_PointPaintFill: "
  ...
  if (pv != _pVolume) {  // normally happens just once after a SetState()
    if (_pVolume == 0) { // SetState() sets _pVolume==0
      ...
      if (_pVolumeName != _pVolume->GetName()) {
        G4cerr << "Warning: actual volume at " << _fixedPos << " is "
               << _pVolume->GetName() << ", not equal to expected volume "
               << _pVolumeName << " in GLG4PosGen_PointPaintFill." << G4endl;
      }
    }
    else {
      G4cerr << "Warning: actual volume at " << _fixedPos
             << " has changed! Now " << pv->GetName() << ", was "
             << _pVolume->GetName() << " in GLG4PosGen_PointPaintFill."
             << G4endl;
      _pVolume = pv;
  ...
  G4double x0, dx, y0, dy, z0, dz;             // for bounding box of solid
  {
    ...
    dz = tmax - z0;
    if (_boundingBoxVolume <= 0.0) _boundingBoxVolume = dx * dy * dz;
    else if (fabs(dx * dy * dz / _boundingBoxVolume - 1.0) >
             1e-6) { // paranoia check
  ...
  if (_mode == kFill) {
    ...
    for (int iloop = 0, jloop = 0; /* test inside */; iloop++) {
      do {
        rpos =
            G4ThreeVector(x0 + dx * G4UniformRand(), y0 + dy * G4UniformRand(),
                          z0 + dz * G4UniformRand()); // uniform in bounding box
        jloop++;
        if (jloop >= 100000) {
          G4cerr << "GLG4PosGen_PointPaintFill::GeneratePosition(): " << iloop
                 << "," << jloop << " loops spent looking for point in "
                 << _pVolumeName;
      ...
      G4VPhysicalVolume * pvtest = gNavigator->LocateGlobalPointAndSetup(
          rpos, 0, true); // fast check mode
      if (_material == 0) {
  ...
  //
    ...
    for (int iloop = 0, jloop = 0; /* test inside */; iloop++) {
      ...
      if (iint >= _intercepts.size()) {
        double Rsphere =
            0.50001 * sqrt(dx * dx + dy * dy + dz * dz) + kCarTolerance;
        G4ThreeVector sphere_center(x0 + 0.5 * dx, y0 + 0.5 * dy,
                                    z0 + 0.5 * dz);
          ...
          for (int isafety = 0; isafety < 20; isafety++) {
            ...
            if (dist <= 0.0) {
              G4cerr << "GLG4PosGen_PointPaintFill: strange DistanceToIn "
                     << dist << " on " << _pVolumeName << " at " << spos
                     << " in dir " << raydir << " loop " << isafety << G4endl;
            }
            if (dist > kCarTolerance) {
              spos += dist * raydir;
              if (_thickness == 0.0) _intercepts.push_back(spos);
              else
...
void GLG4PosGen_PointPaintFill::SetState(G4String newValues)
{
  Strip(newValues);
  ...
  if (newValues == "volume?") {
    if (_mode != kFill || _ntried <= 0) {
      ...
      G4cout << "  bounding box volume: " << _boundingBoxVolume / meter3
             << " m^3\n";
...
G4String GLG4PosGen_PointPaintFill::GetState()
{
  ostringstream os;

  if (_pVolumeName.length() == 0) {
    G4cerr << "Warning: zero-length volume name caught in "
           << "GLG4PosGen_PointPaintFile::GetState()" << G4endl;
    _pVolumeName = "!";
  }
  ...
  if (_mode == kFill) {
    ...
    os << " paint " << _pVolumeName << ' ' << _thickness << ' '
       << _materialName;
  }
                                                ...
                                                double dt)
{
  ...
  while (pp != NULL &&
         abs(pp->GetPDGcode()) >= (GLG4VertexGen_HEPEvt::kPDGcodeModulus *
  ...
  if (dist_to_in < kInfinity) {
    ...
    for (G4PrimaryVertex * v = argVertex; v != NULL; v = v->GetNext()) {
      if (v->GetT0() > max_chain_time) { // reached clip point of chain?
        G4cerr << "GLG4PosGen_Cosmic::GenerateVertexPositions: Warning, "
                  "vertex time exceeds clip, but splitting not supported for "
                  "cosmics\n"
                  "\t t0="
    ...
    for (G4PrimaryVertex * v = argVertex; v != NULL; v = v->GetNext()) {
      for (G4PrimaryParticle * pp = v->GetPrimary(0); pp != NULL;
           pp = pp->GetNext()) {
...
void GLG4PosGen_Cosmic::SetState(G4String newValues)
{
  ...
  if (newValues.length() == 0) {
    ...
    G4cout << "Format of argument to GLG4PosGen_Cosmic::SetState: \n"
              " \"width_mm height_mm\"\n"
              " width_mm  == width of rectangular area normal to incident "
              "direction\n"
              " height_mm == height of rectangular area normal to incident "
              "direction\n"
