/** @file GLG4PMTOpticalModel.cc
    ...
    $Id: GLG4PMTOpticalModel.cc,v 1.2 2013/11/09 23:48:54 jslee Exp $
...
GLG4PMTOpticalModel::GLG4PMTOpticalModel(G4String modelName,
    ...
    : G4VFastSimulationModel(modelName,
                             envelope_phys->GetLogicalVolume()->GetRegion())
{
  _verbosity = 0;
  _luxlevel = 3;
  ...
  if (glass_pt == NULL)
    G4Exception("GLG4PMTOpticalModel: glass lacks a properties table!", "",
                FatalException, "");

  _rindex_glass = glass_pt->GetProperty("RINDEX");
  if (_rindex_glass == NULL)
    G4Exception("GLG4PMTOpticalModel: glass does not have RINDEX!", "",
                FatalException, "");
  ...
  if (pc_log_surface == NULL)
    G4Exception("GLG4PMTOpticalModel: no photocathode logical surface!?!", "",
                FatalException, "");
  ...
  G4OpticalSurface * pc_opsurf =
      (G4OpticalSurface *)(pc_log_surface->GetSurfaceProperty());
#endif

  if (pc_opsurf == NULL)
    G4Exception("GLG4PMTOpticalModel: no photocathode optical surface!?!", "",
                FatalException, "");
  G4MaterialPropertiesTable * pc_pt = pc_opsurf->GetMaterialPropertiesTable();
  if (pc_pt == NULL)
    G4Exception("GLG4PMTOpticalModel: photocathode lacks a properties table!",
                "", FatalException, "");
  ...
  if (_rindex_photocathode == NULL)
    G4Exception("GLG4PMTOpticalModel: photocathode does not have RINDEX!", "",
                FatalException, "");

  _kindex_photocathode = pc_pt->GetProperty("KINDEX");
  if (_kindex_photocathode == NULL)
    G4Exception("GLG4PMTOpticalModel: photocathode does not have KINDEX!", "",
                FatalException, "");

  _thickness_photocathode = pc_pt->GetProperty("THICKNESS");
  if (_thickness_photocathode == NULL)
    G4Exception("GLG4PMTOpticalModel: photocathode does not have THICKNESS!",
                "", FatalException, "");

  _efficiency_photocathode = pc_pt->GetProperty("EFFICIENCY");
  if (_efficiency_photocathode == NULL)
    G4Exception("GLG4PMTOpticalModel: photocathode does not have EFFICIENCY!",
                "", FatalException, "");
...
G4bool GLG4PMTOpticalModel::ModelTrigger(const G4FastTrack & fastTrack)
{
  ...
  if (fastTrack.GetPrimaryTrackLocalPosition().z() > kCarTolerance) return true;
                               ...
                               G4FastStep & fastStep)
{
  ...
  // so we do this:
  {
    ...
    if (ipmt < 0) {
      G4Exception(
          "GLG4PMTOpticalModel: could not find envelope -- where am I !?!", "",
          FatalException, "");
    }
  }
...
#ifdef G4DEBUG
  if (_verbosity >= 10) {
    G4cout << "=> lam, n1, n2: " << _wavelength / nm << " " << _n1 << " "
           << _n2comp << G4endl;
