...
// $Id: GLG4TestSolid.cc,v 1.2 2013/11/09 23:48:54 jslee Exp $
//
/*
  class GLG4TestSolid
...
{
  errstr << G4std::ends;
  const char *cp= errstr.str().c_str();
  if (cp == NULL) {
    return G4String("");
  }
  else {
    G4String rv((const char *)cp);
    //errstr.freeze(0);
    return rv;
  }
}
...
G4String  GLG4TestSolid::Test(const G4VSolid &s, G4int npair)
{
  ...
  G4double myCheckTolerance= G4std::max(G4GeometryTolerance::GetInstance()->GetRadialTolerance(),
				     ...
				     G4double myCheckTolerance)
{
  ...
  for (int idir=0; idir<=1; idir++) {
    ...
    if (s.Inside(p1 + 2*G4GeometryTolerance::GetInstance()->GetRadialTolerance()*e[idir]) != kSurface) {
