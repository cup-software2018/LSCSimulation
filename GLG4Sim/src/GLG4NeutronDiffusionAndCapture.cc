/** @file
    ...
    $Id: GLG4NeutronDiffusionAndCapture.cc,v 1.3 2013/11/14 02:24:35 jslee Exp $
  ...
  : G4VProcess(aName)
{
  ...
  if(_generator == 0){
    G4String msg = "no GLG4PrimaryGeneratorAction instance.";
    G4Exception("GLG4NeutronDiffusionAndCapture::GLG4NeutronDiffusionAndCapture", "", FatalException, msg);
  }

  _neutronCaptureProcess = (G4NeutronCaptureProcess*)(G4ProcessTable::GetProcessTable()->FindProcess("LCapture","neutron"));
  if(_neutronCaptureProcess == 0){
    G4String msg = "could not find neutron capture";
    G4Exception("GLG4NeutronDiffusionAndCapture::GLG4NeutronDiffusionAndCapture", "", FatalException, msg);
  }
}
...
G4double GLG4NeutronDiffusionAndCapture::PostStepGetPhysicalInteractionLength(const G4Track& aTrack,
									      ...
									      G4ForceCondition* condition)
{
  ...
  *condition = NotForced;
  ...
  if(aTrack.GetKineticEnergy() < 100.*eV){
    G4MaterialPropertiesTable * materialPropertiesTable = aTrack.GetMaterial()->GetMaterialPropertiesTable();
    if(materialPropertiesTable){
      G4MaterialPropertyVector * captureTimeVector = materialPropertiesTable->GetProperty("NEUTRON_CAPTURE_TIME");
...
G4VParticleChange* GLG4NeutronDiffusionAndCapture::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  if(materialPropertiesTable){
    G4MaterialPropertyVector * mpv = materialPropertiesTable->GetProperty("NEUTRON_CAPTURE_TIME");;
    if(mpv)
    ...
    G4cerr << "GLG4NeutronDiffusionAndCapture::PostStepDoIt: error, no material properties table for " 
	   << material->GetName() << G4endl;
  }
  ...
  G4VParticleChange * nChange = _neutronCaptureProcess->PostStepDoIt(nTrack, aStep);
