/** @file LSCPMTOpticalModel.cc
  ...
  $Id: LSCPMTOpticalModel.cc,v 1.2 2005/08/31 17:49:58 volsung Exp $
...
G4UIdirectory * LSCPMTOpticalModel::fgCmdDir = nullptr;
double LSCPMTOpticalModel::surfaceTolerance = 0.0;
  ...
  : G4VFastSimulationModel(modelName, envelope_region)
{
  ...
  if (_rindex_glass == nullptr) {
    G4Exception(__FILE__, "Bad Properties", FatalException,
                "LSCPMTOpticalModel: glass does not have RINDEX!");
  }
  ...
  if (pc_opsurf == nullptr) {
    G4Exception(__FILE__, "Bad Properties", FatalException,
                "LSCPMTOpticalModel: no photocathode optical surface!?!");
  }

  G4MaterialPropertiesTable * pc_pt = pc_opsurf->GetMaterialPropertiesTable();
  if (pc_pt == nullptr) {
    G4Exception(__FILE__, "Bad Properties", FatalException,
                "LSCPMTOpticalModel: photocathode lacks a properties table!");
  }
...
// -- see also Geant4 docs
G4bool LSCPMTOpticalModel::ModelTrigger(const G4FastTrack & fastTrack)
{
                              ...
                              G4FastStep & fastStep)
{
  ...
  for (iloop = 0; iloop < max_iloop; iloop++) {
    ...
    if (_verbosity >= 2) {
      G4cout << " " << iloop << " dist=" << dist << " newpos=" << pos << endl;
    }
...
void LSCPMTOpticalModel::CalculateCoefficients()
...
void LSCPMTOpticalModel::SetNewValue(G4UIcommand * command, G4String newValues)
{
  ...
  if (commandName == "verbose") {
    ...
    G4cerr << "No PMTOpticalModel command named " << commandName << endl;
  }
  return;
}

G4String LSCPMTOpticalModel::GetCurrentValue(G4UIcommand * command)
{
  G4String commandName = command->GetCommandName();
  if (commandName == "verbose") {
    char outbuff[64];
...
int LSCPMTOpticalModel::GetPMTID(const G4FastTrack & fastTrack)
{
  int iDepth;
