...
#ifndef LSCCerenkov_h
#define LSCCerenkov_h 1
...
class LSCCerenkov : public G4VProcess, public G4UImessenger {
public:
  explicit LSCCerenkov(const G4String & processName = "Cerenkov",
                       G4ProcessType type = fElectromagnetic);
  ~LSCCerenkov();

  explicit LSCCerenkov(const LSCCerenkov & right);

  LSCCerenkov & operator=(const LSCCerenkov & right) = delete;
...
private:
  ...
  G4UIcmdWithAString * fCerenkovOnCmd;
  G4int fIsCerenkovOn;
};

inline G4bool LSCCerenkov::GetTrackSecondariesFirst() const
{
  return fTrackSecondariesFirst;
}

inline G4double LSCCerenkov::GetMaxBetaChangePerStep() const
{
  return fMaxBetaChange;
}

inline G4int LSCCerenkov::GetMaxNumPhotonsPerStep() const
{
  return fMaxPhotons;
}

inline G4bool LSCCerenkov::GetStackPhotons() const { return fStackingFlag; }

inline G4int LSCCerenkov::GetNumPhotons() const { return fNumPhotons; }

inline G4PhysicsTable * LSCCerenkov::GetPhysicsTable() const
{
