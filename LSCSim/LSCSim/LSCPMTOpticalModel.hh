/** @file LSCPMTOpticalModel.hh
    ...
    $Id: LSCPMTOpticalModel.hh,v 1.1 2005/08/30 19:55:22 volsung Exp $
...
#ifndef __LSCPMTOpticalModel_hh__
#define __LSCPMTOpticalModel_hh__
...
class LSCPMTOpticalModel : public G4VFastSimulationModel, public G4UImessenger {
public:
  //-------------------------
  ...
  LSCPMTOpticalModel(G4String, G4Region *, G4LogicalVolume * body,
  ...
  // Note: There is no LSCPMTOpticalModel(G4String) constructor.
  ~LSCPMTOpticalModel();
  ...
  void SetEfficiencyCorrection(std::map<int, double> _EffiCorr)
  {
    ...
    std::cout << GetName() << ": Individual efficiency correction table set"
              << std::endl;
  }
  void DumpEfficiencyCorrectionTable()
  {
    std::cout << "Individual correction table for the PMT efficiencies of "
              << GetName() << ":\nPMT ID  corr. factor" << std::endl;
    for (std::map<int, double>::iterator iter = EfficiencyCorrection.begin();
         iter != EfficiencyCorrection.end(); iter++) {
      std::cout << iter->first << "," << iter->second << std::endl;
    }
  }
  void fillPMTVector(double code, double A, double An, double T, double R,
                     double collection_eff, double N_pe, double x, double y,
...
private:
  ...
  G4double
      _efficiency_correction; // global efficiency correction, default to 1.0
  G4double _dynodeTop;
